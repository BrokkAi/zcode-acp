...
| `ZCODE_HOME`                       | `~/.zcode`          | Directory that replaces `~/.zcode` as the ZCode data root. Both the credentials/provider config (`<dir>/v2/config.json`) and the lazy-session alias store (`<dir>/v2/acp-lazy-sessions.json`) are read from it. Set it before starting the bridge — the credentials path is resolved once at startup.                                                                                                                                                                            |
...
| `ZCODE_DISALLOWED_TOOLS`           | _(unset)_           | Tools the backend must not use, passed verbatim to `zcode app-server` as `--disallowed-tools <value>`. Give it the backend's own list format (comma- or space-separated tool names, e.g. `Bash,Write`). Unset omits the flag entirely, so the backend's default tool set applies.                                                                                                                                                        |
...
<a href="https://github.com/william0wang/zcode-acp/graphs/contributors">
  ...
  <img src="https://contrib.rocks/image?repo=william0wang/zcode-acp&v=0.37.3" alt="Contributors" /> <!-- x-release-please-version -->
</a>
